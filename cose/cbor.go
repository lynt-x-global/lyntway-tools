...
// decodeBytes reads a byte string.
func decodeBytes(in []byte) ([]byte, []byte, error) {
	major, length, rest, err := decodeHead(in)
	if err != nil {
		return nil, nil, err
	}
	if major != majorBytes {
		return nil, nil, fmt.Errorf("%w: expected a byte string", ErrMalformed)
	}
